...
public class Island2Generation : MonoBehaviour {
    ...
    void Awake()
    {
        ...
        nextPosition = new Vector3((float)Random.Range(-16,16),.2f, (float)Random.Range(-60,-10));
		for (int i = 0; i < numOfobjects; i++)
        {
            Recycle();
        }
    ...
    {	float offset = (float)Random.Range(minGap, maxGap);
        ...
        road.localPosition =  new Vector3((float)Random.Range(-12,12), .2f, nextPosition.z);
